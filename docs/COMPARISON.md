...
## Annex: Parity harness

`scratch/parity_check.py` is a development-only verification tool (not part
of the shipped port). It parses `add_rule`/`execute`/`execute_with_string`
calls out of upstream liblognorm's `tests/*.sh` fixtures — treating them as
data, never executing the shell files — replays each runnable case against
both a real `lognormalizer` binary built from upstream C and this port's CLI,
and diffs the two JSON outputs semantically.

To run it, clone `rsyslog/liblognorm` and build its `lognormalizer` binary,
build this port's CLI (`dotnet build`), then point the harness at both via
environment variables (all optional — see the script header for defaults):

```shell
export PARITY_TESTS_DIR=/path/to/liblognorm/tests
export PARITY_C_BIN=/path/to/lognormalizer
export PARITY_CSHARP_CLI_DLL=src/tools/LogNormalizer.Cli/bin/Debug/net10.0/lognormalizer.dll
