...
/// </para>
/// </summary>
internal static class PdagWalker
{
    ...
    /// <summary>Parse a message against a snapshot (port of ln_normalize).</summary>
    public static int Parse(ParseContext ctx, CompiledPdag snap, string str, out FieldCollector fields)
    {
        fields = new FieldCollector();
        return ParseCore(ctx, snap, str, fields);
    }

    /// <summary>Parse and materialize the result as a <see cref="JsonObject"/>.</summary>
    public static int Parse(ParseContext ctx, CompiledPdag snap, string str, out JsonObject result)
    {
         ...
         * callers who never touch the flat ParseResult API */
        var fields = FieldCollector.RentScratch();
        try
        {
            var r = ParseCore(ctx, snap, str, fields);
    ...
    /// <summary>
    /// Recursive step of the walker (port of ln_normalizeRec).
    /// </summary>
    /// <param name="npb">parse parameter block (message, state, snapshot)</param>
    ...
    public static int ParseRec(Npb npb, int nodeIdx, int offs, bool bPartialMatch,
...
#pragma warning restore IDE0059 // Unnecessary assignment of a value
        ...
        for (var iprs = node.EdgeStart; iprs < edgeEnd && r != 0; ++iprs)
        {
            ...
            if (localR == 0)
            {
                ...
                r = ParseRec(npb, prs.TargetNode, parsedTo, bPartialMatch, fields, ref endNode,
                ...
                if (r == 0)
                {
                    ...
                    if ((npb.Ctx.Options & ParseOptions.AddRule) != 0)
                    {
         ...
         * length from this value after their nested ParseRec call. */
    ...
    private static void AddRuleMetadata(Npb npb, FieldCollector fields, TerminalInfo endNode)
    {
        ...
        if ((ctx.Options & ParseOptions.AddRule) != 0)
        {
        ...
        if ((ctx.Options & ParseOptions.AddRuleLocation) != 0)
        {
            metaRule ??= new JsonObject();
    ...
    private static void MaterializeValue(Npb npb, in CompiledEdge prs, int offs, ref FieldValue value)
    {
        ...
        if (prs.PrsId == ParserTable.CustomTypeId)
        {
            ...
            var endNode = NoNode;
            npb.ParsedTo = offs;
            var r = ParseRec(npb, npb.Snap.TypeRoots[prs.CustomTypeIdx], offs, bPartialMatch: true,
    ...
    private static int ParseCore(ParseContext ctx, CompiledPdag snap, string str, FieldCollector fields)
    {
        ...
        var endNode = NoNode;
        var r = ParseRec(npb, snap.RootNode, 0, bPartialMatch: false, fields, ref endNode,
        ...
        string? parserName)
    {
        ...
        if (prs.PrsId == ParserTable.CustomTypeId)
        {
            ...
            if (failOnDuplicate)
            {
                ...
                r = ParseRec(npb, npb.Snap.TypeRoots[prs.CustomTypeIdx], offs, bPartialMatch: true,
