...
/// node/edge arena. Parsing only ever reads a snapshot, so a context can
/// be reloaded by compiling a new snapshot and atomically swapping it in while
/// in-flight parses finish on the old one.
/// </summary>
internal sealed class CompiledPdag
{
    ...
    /// <see cref="ParseOptions.CollectStats"/>. Increments are unsynchronized
