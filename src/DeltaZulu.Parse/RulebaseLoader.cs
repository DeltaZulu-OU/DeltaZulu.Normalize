...
/// engine (a separate, older, non-PDAG implementation with its own rulebase dialect) is
...
internal static class RulebaseLoader
{
    ...
    /// </summary>
    public static int Load(ParseContext ctx, string path)
    {
        /* a path that exists as given wins over the DeltaZulu.Parse_RULEBASES
    ...
    /// </summary>
    public static int LoadDirectory(ParseContext ctx, string directory, string searchPattern, bool recursive)
    {
    ...
    public static int LoadFile(ParseContext ctx, string path)
    {
    ...
    public static int LoadString(ParseContext ctx, string rulebase)
    {
        var savedFile = ctx.ConfFile;
        var savedLine = ctx.ConfLineNumber;
    ...
    /// </summary>
    private static int AddFieldDescr(ParseContext ctx, ref Pdag pdag, string rule, ref int i)
    {
        ++i; /* eat '%' */
    ...
    /// </summary>
    private static int AddSampToTree(ParseContext ctx, string rule, ref Pdag dag, JsonArray? tagBucket)
    {
        var i = 0;
        while (i < rule.Length)
        {
    ...
    private static bool GetTypeName(ParseContext ctx, string line, ref int offs, out string typeName)
    {
    ...
    private static bool ParseLegacyFieldDescr(ParseContext ctx, string rule, ref int i, out JsonObject? config)
    {
        config = null;
    ...
    /// </summary>
    private static int ParseLiteral(ParseContext ctx, ref Pdag pdag, string rule, ref int i)
    {
        var sb = new StringBuilder();
        while (i < rule.Length)
        {
    ...
    private static int ProcessAnnotate(ParseContext ctx, string line, int offs)
    {
    ...
    private static int ProcessInclude(ParseContext ctx, string line, int offs)
    {
    ...
    private static int LoadIncludedRulebase(ParseContext ctx, string includePath)
    {
    ...
    private static int ProcessLine(ParseContext ctx, string line)
    {
        var eq = line.IndexOf('=');
    ...
    private static int ProcessRule(ParseContext ctx, string line, int offs)
    {
    ...
    private static int ProcessType(ParseContext ctx, string line, int offs)
    {
    ...
    /// </summary>
    private static string? ReadLogicalLine(ParseContext ctx, string text, ref int pos, bool checkRunaway)
    {
    ...
    {
        var rbLib = Environment.GetEnvironmentVariable("DeltaZulu.Parse_RULEBASES")
    ...
    private static int RunLoad(ParseContext ctx, string text, bool checkRunaway)
    {
        var pos = 0;
        while (true)
        {
    ...
    private static int SampLoad(ParseContext ctx, string file)
    {
